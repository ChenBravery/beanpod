...
import org.beanpod.auth.entity.MenuPermission;

/**
 * 
 * MenuPermission鏁版嵁鎿嶄綔鎺ュ彛绫�
 * 
 **/
@Mapper
public interface IMenuPermissionDao {

	@Results(id="menuPermission",value={
	...
	/**
	 * 
	 * 鎵归噺娣诲姞MenuPermission
	 * 
	 **/
	@InsertProvider(type = SqlProvider.class, method = "insertAll")
	int insertAll( List<MenuPermission> menuPermissionList );
	...
	/**
	 * 
	 * 鎵归噺淇敼MenuPermission
	 * 
	 **/
	//int batchUpdate( @Param("menuPermission")MenuPermission menuPermission, @Param("params") Map<String, ?> params );
	...
	MenuPermission selectById ( String id );
    ...
    List<MenuPermission> selectList ( MenuPermission menuPermission );
	 ...
	 * 
	 **/
	//List<MenuPermission> selectListByMap ( Map<String, ?> params );
    
	/**
	 * 
	 * 鏌ヨMenuPermission鍒楄〃澶у皬
	 * 
	 **/
	//Integer selectListSize ( MenuPermission menuPermission );
	...
	class SqlProvider {
		public String insert(MenuPermission menuPermission) {
			return new SQL(){{
				INSERT_INTO("t_sys_menu_permission");
	    ...
	    public String insertAll( Map<String, List<MenuPermission>> map ) {
			boolean genId = false;
	        List<MenuPermission> list = map.get("list");
	        StringBuilder sb = new StringBuilder();
	        sb.append("INSERT INTO t_sys_menu_permission (");
		...
		public String updateById( MenuPermission menuPermission ) {
			return new SQL(){{
				UPDATE("t_sys_menu_permission");
				if ( menuPermission.getId() != null) {
					SET("id = #{id}");
		...
		public String selectById(String id) {
			return new SQL(){{      
				SELECT("*");
				FROM("t_sys_menu_permission");
				WHERE("id = #{id}");
				WHERE("deleted = 0");      
			}}.toString();
		}
		
		public String selectList ( MenuPermission menuPermission ) {
			return new SQL(){{      
				SELECT("*");
				FROM("t_sys_menu_permission");
				if ( menuPermission.getId() != null && !menuPermission.getId().isEmpty() ) {
					WHERE("id = #{id}");
				}
				if ( menuPermission.getMenuId() != null && !menuPermission.getMenuId().isEmpty() ) {
					WHERE("menu_id = #{menuId}");
				}
				if ( menuPermission.getPermissionId() != null && !menuPermission.getPermissionId().isEmpty() ) {
